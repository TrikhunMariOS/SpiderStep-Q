...
"""Gait Test 4: Bezier crawl gait for custom Spider Robot.
...
Changes from Test 3:
    ...
    - Uses cubic Bezier curve for swing phase foot trajectory.
    - Keeps one-leg-at-a-time crawl gait.

Usage:
    isaaclab.bat -p Gait_Test4_Bezier_Crawl.py
"""
...
GAIT_FOOT_CONTACT_BODY = {
    "FL": (-155.0, -110.0, -66.34),
    "FR": (-155.0,  110.0, -66.34),
    "RL": ( 155.0, -110.0, -66.34),
    "RR": ( 155.0,  110.0, -66.34),
}
...
# Bezier swing shape tuning.
# These values control the two middle control points of the swing curve.
# Larger values make the foot rise earlier/stay higher longer.

## my setting walk
BEZIER_LIFT_FRONT_RATIO = 0.6
BEZIER_LIFT_BACK_RATIO = 0.65

# normal walk
# BEZIER_LIFT_FRONT_RATIO = 0.20
# BEZIER_LIFT_BACK_RATIO = 0.80

#Dudan walk
# BEZIER_LIFT_FRONT_RATIO = 0.15
# BEZIER_LIFT_BACK_RATIO = 0.75
...
# Optional extra forward shaping.
# 0.0 = normal. Positive value pushes the middle control points slightly forward.
BEZIER_FORWARD_BIAS = 0.0
...
def smoothstep(u: float):
    ...
    Used for stance phase only.
    """
    u = max(0.0, min(1.0, u))
    return u * u * (3.0 - 2.0 * u)


def bezier3(p0, p1, p2, p3, u: float):
    """Cubic Bezier interpolation.

    p0, p1, p2, p3 can be floats or numpy arrays.
...
def foot_trajectory_body(leg: str, phase: float):
    """Generate foot-center target for one leg using Bezier swing.

    phase < SWING_RATIO:
        swing phase:
            uses cubic Bezier curve P0 -> P3.

    phase >= SWING_RATIO:
        stance phase:
            foot stays on ground and moves back slowly.

    Coordinate meaning:
        x = forward/backward in body frame
        y = left/right in body frame
        z = up/down in body frame
    """

    x0, y0, z0 = GAIT_FOOT_BODY[leg]

    # In this robot, front is approximately -X.
    ...
    if phase < SWING_RATIO:
        ...
        # P0: take-off point, behind the neutral point.
        p0 = np.array([x_back, y0, z0], dtype=np.float64)

        # P1: lift early.
        # Same x as near-back/mid point, z is raised.
        p1_x = x_back + (x_front - x_back) * BEZIER_LIFT_FRONT_RATIO
        p1_x += FORWARD_SIGN_X * BEZIER_FORWARD_BIAS
        p1 = np.array([p1_x, y0, z0 + STEP_HEIGHT], dtype=np.float64)

        # P2: stay high while moving toward front.
        p2_x = x_back + (x_front - x_back) * BEZIER_LIFT_BACK_RATIO
        p2_x += FORWARD_SIGN_X * BEZIER_FORWARD_BIAS
        p2 = np.array([p2_x, y0, z0 + STEP_HEIGHT], dtype=np.float64)

        # P3: landing point, in front of the neutral point.
        p3 = np.array([x_front, y0, z0], dtype=np.float64)

        p = bezier3(p0, p1, p2, p3, u)
        return (float(p[0]), float(p[1]), float(p[2]))

    else:
        # Stance phase: foot is on the ground and slowly returns backward.
...
def main():
    ...
    print("[INFO]: Gait Test 4: Bezier crawl gait.")
